...
class SendEmailSpec extends MailServerSpec {
  ...
  "email" should {
    "send mime email" in {
      ...
      println(message)
      message.Bcc.toList.flatMap(_.map(_.Address)) === email.bcc.toList.flatMap(_.toList.map(_.address))
      message.Cc.toList.flatMap(_.map(_.Address)) === email.cc.toList.flatMap(_.toList.map(_.address))
