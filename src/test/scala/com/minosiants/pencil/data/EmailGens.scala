...
trait EmailGens {
  ...
  val subjectGen: Gen[Subject]              = Gen.asciiPrintableStr.map(Subject)
  val textBodyGen: Gen[Body.Ascii]          = Gen.asciiPrintableStr.map(Body.Ascii)
  val htmlBodyGen: Gen[Body.Html]           = Gen.asciiPrintableStr.map(Body.Html)
  val utf8BodyGen: Gen[Body.Utf8]           = Gen.asciiPrintableStr.map(Body.Utf8)
