...
enum Code(val value: Int, val description: String):
  ...
  case `450`
      extends Code(450, "Requested mail action not taken: mailbox unavailable")
  case `451`
      extends Code(451, "Requested action aborted: local error in processing")
  case `452`
      extends Code(
        452,
  ...
  case `550`
      extends Code(550, "Requested action not taken: mailbox unavailable")
  ...
  case `553`
      extends Code(553, "Requested action not taken: mailbox name not allow")
...
object Code:
  def code(v: Int): Option[Code] = Code.values.find(_.value == v)
